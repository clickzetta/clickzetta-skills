...
clean-lakehouse-doc:
	@echo "🧹 Cleaning $(LAKEHOUSE_DOC_DST) (preserving skills/)..."
	@find $(LAKEHOUSE_DOC_DST) -mindepth 1 -maxdepth 1 ! -name 'skills' -exec rm -rf {} +
	@rm -f $(LAKEHOUSE_DOC_SKILL)

sync-lakehouse-doc: clean-lakehouse-doc
	@echo "📚 Syncing lakehouse-doc from $(LAKEHOUSE_DOC_SRC)..."
	@mkdir -p $(LAKEHOUSE_DOC_DST)
	rsync -a --prune-empty-dirs --include='*/' --exclude='skills/***' --exclude='skills/' --exclude='RN[-_]*.md' --exclude='rn[-_]*.md' --exclude='CLAUDE.md' --exclude='AGENTS.md' --include='*.md' --include='llms-*.txt' --exclude='.*' --exclude='*' $(LAKEHOUSE_DOC_SRC)/ $(LAKEHOUSE_DOC_DST)/
	@echo "✅ Synced $$(find $(LAKEHOUSE_DOC_DST) -type f ! -path '*/skills/*' | wc -l | tr -d ' ') doc files"
