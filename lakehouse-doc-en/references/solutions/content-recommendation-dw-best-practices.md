...
```sql
-- 先创建 Storage Connection（OSS 访问凭据）
...
-- 创建 OSS PIPE（LIST_PURGE：导入后删除原文件，避免重复导入）
...
WHERE MATCH_ALL(description, '开放世界');
```

```
content_id   | title                    | description
-------------+--------------------------+----------------------
GAME_292030  | The Witcher 3: Wild Hunt | 开放世界角色扮演游戏，剧情丰富
GAME_1091500 | Cyberpunk 2077           | 未来世界开放世界RPG，赛博朋克风格
GAME_271590  | Grand Theft Auto V       | 开放世界动作冒险游戏
```
...
CREATE TABLE IF NOT EXISTS best_practice_content_rec.bronze_content_embedding (
    ...
    description STRING,
    tags        STRING,
    embedding   VECTOR(128)    -- 由 External Function 生成的 128 维 Embedding
);
```
...
```sql
-- 假设已部署 External Function：text2vec
-- 调用示例（在运营脚本或 ZettaPark Task 中执行）
...
FROM (
    SELECT 'GAME_730' AS content_id, 'Counter-Strike 2' AS title,
           '多人竞技射击游戏，玩家加入恐怖分子或反恐精英阵营对抗' AS description,
