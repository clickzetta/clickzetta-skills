...
1. download_raw_data.sh: Download Public URL Parquet files to local. This article uses NYC Taxi Data as an example, containing over 450 files and 60GB of data (Parquet format).

2. initialize_database.sh: Create Singdata Lakehouse schema and tables via Lakehouse SQLLine commands.

3. Import taxi and FHV data: Convert Parquet files to CSV files using R, then import data from local CSV files into Singdata Lakehouse tables using the Lakehouse SQLLine COPY command, supporting batch import of multiple files.
...
[Full Script](https://github.com/clickzetta/nyc-taxi-data-clickzetta/blob/master/readme.md)
