...
If this difference exceeds the configured skip duration, the instance is immediately marked as a dry-run skip and its status is set to succeeded.

For example: an instance is scheduled to run every 5 minutes, with a skip duration of 6 minutes. For an instance scheduled at 14:05, if upstream instance delays cause it to not enter the running state until 14:12, the interval is 7 minutes — exceeding the configured 6 minutes — so the system immediately performs a dry-run skip and marks the instance as succeeded.
