...
询问用户两个问题：(1) Review 范围是什么 — 全量 Review（发现所有问题，给出完整报告）、专项诊断（只看某类问题，如任务依赖、DT 刷新失败、数据不一致），还是快速健康检查（只看 P0 问题，5 分钟内出结论）？(2) 执行权限是什么 — 可读写（可以执行修复操作，推荐）还是只读（只能查，输出报告不执行修复）？管道入口（业务域/任务名/schema/表名）和已知症状可在用户回答后追问，或从上下文推断。
