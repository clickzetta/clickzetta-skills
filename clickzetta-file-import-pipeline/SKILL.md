...
询问用户两个问题：(1) 文件来源是什么 — HTTP/HTTPS URL（自动下载）、本地文件（上传到 User Volume）、已在 Volume 上（提供 Volume 名称和路径），还是外部 Volume（OSS/S3/COS）？(2) 写入模式是什么 — create（自动建表，表不存在时推断 schema）、append（追加到已有表）还是 overwrite（清空已有表再写入）？
